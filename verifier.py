...
def format_checker(inputfile, outputfile, modelfile):
    ...
    models = parse_outputfile(modelfile, T)
    for i, (output, model) in enumerate(zip(outputs, models)):
        if outputs[0] != models[0] and outputs[1] != models[1]:
            print("Not same in testcase #{}: Expected ", (models[0], models[1]), ", found ", (outputs[0], outputs[1]))
            exit(0)
    print("... Correct")
...
if __name__ == '__main__':
    format_checker(sys.argv[1], sys.argv[2], sys.argv[3])
